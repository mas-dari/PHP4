...
if(empty($_GET)) {
	...
	if ($urltoken == ''){
		...
		try{
			...
			$sql = "SELECT mail FROM gs_pre_user WHERE urltoken=(:urltoken) AND flag =0 AND date > now() - interval 24 hour";
           $stm = $pdo->prepare($sql);
...
if(isset($_POST['btn_submit'])){
	...
	//ここでデータベースに登録する
	try{
		$sql = "INSERT INTO gs_user_table (name,password,mail,status,created_at,updated_at) VALUES (:name,:password_hash,:mail,1,now(),now())";
       $stm = $pdo->prepare($sql);
		...
		$sql = "UPDATE gs_pre_user SET flag=1 WHERE mail=:mail";
		$stm = $pdo->prepare($sql);
		//プレースホルダへ実際の値を設定する
