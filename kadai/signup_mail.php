...
if (isset($_POST['submit'])) {
   ...
   if (empty($_POST['mail'])) {
       ...
       //DB確認        
       $sql = "SELECT id FROM gs_user_table WHERE mail=:mail";
       $stm = $pdo->prepare($sql);
   ...
   if (count($errors) === 0){
       ...
       $url = "http://localhost/gs_code/PHP4/kadai/signup.php?urltoken=".$urltoken;

       //ここでデータベースに登録する
       try{
           //例外処理を投げる（スロー）ようにする
           $sql = "INSERT INTO gs_pre_user (urltoken, mail, date, flag) VALUES (:urltoken, :mail, now(), '0')";
           $stm = $pdo->prepare($sql);
